...
func TestTransactionDel(t *testing.T) {
	...
	store.txnHandler.tw = &mockTransactionWorkerDel{}
	...
	Convey("Test mongodb token store", t, func() {
		...
		Convey("Test transaction succeed", func() {

			err := store.txnHandler.runTransactionDelete(context.TODO(), info, basic)

			So(err, ShouldBeNil)
			So(len(record), ShouldEqual, 9)
			So(record[0], ShouldEqual, "insertTokenTransactionData")
			So(record[1], ShouldEqual, "insertTokenTransactionData")
			So(record[2], ShouldEqual, "insertBasicTransactionData")
			So(record[3], ShouldEqual, "removeTokenData")
			So(record[4], ShouldEqual, "removeTransactionData")
			So(record[5], ShouldEqual, "removeTokenData")
			So(record[6], ShouldEqual, "removeTransactionData")
			So(record[7], ShouldEqual, "removeBasicData")
		...
		Convey("Test insertTokenTransactionData accesstoken fail", func() {

			info.Access = "insertTokenTransactionDataAccesstokenFail"

			err := store.txnHandler.runTransactionDelete(context.TODO(), info, basic)

			So(err.Error(), ShouldEqual, "insertTokenTransactionDataAccesstokenFail")
			So(len(record), ShouldEqual, 1)

			info.Access = "accesstoken"
			callNumber = 0
			record = []string{}
		})

		Convey("Test insertTokenTransactionData refreshtoken fail", func() {

			info.Refresh = "insertTokenTransactionDataRefreshtokenFail"

			err := store.txnHandler.runTransactionDelete(context.TODO(), info, basic)

			So(err.Error(), ShouldEqual, "insertTokenTransactionDataRefreshtokenFail")
			So(len(record), ShouldEqual, 2)
			So(record[0], ShouldEqual, "insertTokenTransactionData")
			So(record[1], ShouldEqual, "insertTokenTransactionData")

			info.Refresh = "accessrefresh"
			callNumber = 0
			record = []string{}
		})

		Convey("Test insertTokenTransactionData basictoken fail", func() {

			basic = "insertTokenTransactionDataBasictokenFail"

			err := store.txnHandler.runTransactionDelete(context.TODO(), info, basic)

			So(err.Error(), ShouldEqual, "insertTokenTransactionDataBasictokenFail")
		...
		Convey("Test removeTokenData accesstoken fail", func() {

			info.Access = "removeTokenDataAccesstokenFail"

			err := store.txnHandler.runTransactionDelete(context.TODO(), info, basic)

			So(err.Error(), ShouldEqual, "removeTokenDataAccesstokenFail")
			So(len(record), ShouldEqual, 4)
			So(record[0], ShouldEqual, "insertTokenTransactionData")
			So(record[1], ShouldEqual, "insertTokenTransactionData")
			So(record[2], ShouldEqual, "insertBasicTransactionData")
			So(record[3], ShouldEqual, "removeTokenData")
		...
		Convey("Test removeTransactionData accesstoken fail", func() {

			info.Access = "removeTransactionDataAccesstokenFail"

			_ = store.txnHandler.runTransactionDelete(context.TODO(), info, basic)
		...
		Convey("Test removeTokenData refreshtoken fail", func() {

			info.Refresh = "removeTokenDataRefreshtokenFail"

			err := store.txnHandler.runTransactionDelete(context.TODO(), info, basic)

			So(err.Error(), ShouldEqual, "removeTokenDataRefreshtokenFail")
			...
			info.Refresh = "refreshtoken"
			callNumber = 0
			record = []string{}
		})

		Convey("Test removeTransactionData refreshtoken fail", func() {

			info.Refresh = "removeTransactionDataRefreshtokenFail"
		...
		Convey("Test removeTokenData basictoken fail", func() {

			basic = "removeTokenDataBasictokenFail"

			err := store.txnHandler.runTransactionDelete(context.TODO(), info, basic)

			So(err.Error(), ShouldEqual, "removeTokenDataBasictokenFail")
			So(len(record), ShouldEqual, 8)
			So(record[0], ShouldEqual, "insertTokenTransactionData")
			So(record[1], ShouldEqual, "insertTokenTransactionData")
			So(record[2], ShouldEqual, "insertBasicTransactionData")
			So(record[3], ShouldEqual, "removeTokenData")
		...
		Convey("Test removeTransactionData basictoken fail", func() {

			basic = "removeTransactionDataBasictokenFail"
...
type mockTransactionWorkerDel struct{}

func (mt *mockTransactionWorkerDel) insertBasicData(ctx context.Context, basicData basicData) error {
	record = append(record, "insertBasicData")
	return nil
}

func (mt *mockTransactionWorkerDel) removeBasicData(ctx context.Context, basicDataID string) error {
	record = append(record, "removeBasicData")
	if basicDataID == "removeTokenDataBasictokenFail" {
		return errors.New("removeTokenDataBasictokenFail")
	}
	return nil
}

func (mt *mockTransactionWorkerDel) insertBasicTransactionData(ctx context.Context, txnData transactionData) error {
	record = append(record, "insertBasicTransactionData")
	if txnData.ID == "insertTokenTransactionDataBasictokenFail" {
		return errors.New("insertTokenTransactionDataBasictokenFail")
	}
	return nil
}

func (mt *mockTransactionWorkerDel) insertTokenData(ctx context.Context, tokenData tokenData, collectionName string) error {
	record = append(record, "insertTokenData")
	return nil
}

func (mt *mockTransactionWorkerDel) removeTokenData(ctx context.Context, tokenDataID, collectionName string) error {
	record = append(record, "removeTokenData")
	if tokenDataID == "removeTokenDataAccesstokenFail" {
		return errors.New("removeTokenDataAccesstokenFail")
	} else if tokenDataID == "removeTokenDataRefreshtokenFail" {
		return errors.New("removeTokenDataRefreshtokenFail")
	}
	return nil
}

func (mt *mockTransactionWorkerDel) insertTokenTransactionData(ctx context.Context, txnData transactionData) error {
	callNumber++
	record = append(record, "insertTokenTransactionData")
	if txnData.ID == "insertTokenTransactionDataAccesstokenFail" {
		return errors.New("insertTokenTransactionDataAccesstokenFail")
	} else if txnData.ID == "insertTokenTransactionDataRefreshtokenFail" && callNumber == 2 {
		return errors.New("insertTokenTransactionDataRefreshtokenFail")
	}
	return nil
}

func (mt *mockTransactionWorkerDel) removeTransactionData(ctx context.Context, tokenDataID string) error {
	record = append(record, "removeTransactionData")
	if tokenDataID == "removeTransactionDataAccesstokenFail" {
		return errors.New("removeTransactionDataAccesstokenFail")
	} else if tokenDataID == "removeTransactionDataRefreshtokenFail" {
		return errors.New("removeTransactionDataRefreshtokenFail")
	} else if tokenDataID == "removeTransactionDataBasictokenFail" {
		return errors.New("removeTransactionDataBasictokenFail")
	}
	return nil
}

func (mt *mockTransactionWorkerDel) cleanupTransactionsData(ctx context.Context, service string) error {
	return nil
}
