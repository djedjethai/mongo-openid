...
func (th *transactionHandler) runTransactionDelete(ctx context.Context, ti oauth2.TokenInfo, basic string) (errRET error) {
	...
	// T1 delete accessToken
	accessTxnData := transactionData{
		ID:         ti.GetAccess(),
		TxnID:      txnID,
	...
	// T2 delete refreshToken
	refreshTxnData := transactionData{
		ID:         ti.GetRefresh(),
		TxnID:      txnID,
		Collection: th.tcfg.RefreshCName,
		Service:    th.tcfg.storeConfig.service,
		CreatedAt:  time.Now(),
	}

	// T3 delete basicToken
	...
	// save transaction in txnCollection
	errRET = th.tw.insertTokenTransactionData(ctx, accessTxnData)
	if errRET != nil {
		if !mongo.IsDuplicateKeyError(err) {
			log.Println("T1: Failed add accessTxnData to TxnCName: ", errRET)
			return
		}
	}
	errRET = th.tw.insertTokenTransactionData(ctx, refreshTxnData)
	if errRET != nil {
		if !mongo.IsDuplicateKeyError(err) {
			log.Println("T2: Failed add refreshTxnData to TxnCName: ", errRET)
			return
		}
	}
	...
	if errRET != nil {
		if !mongo.IsDuplicateKeyError(err) {
			log.Println("T3: Failed to add basicTxnData to TxnCName: ", errRET)
			return
		}
	}

	// remove the tokens and if ok delete matching txnData
	errRET = th.tw.removeTokenData(ctx, ti.GetAccess(), th.tcfg.AccessCName)
	if errRET != nil {
		log.Println("T1: Failed to remove accessToken from AccessCName: ", errRET)
		return
	} else {
		err = th.tw.removeTransactionData(ctx, ti.GetAccess())
		if err != nil {
			log.Println("T1: Failed remove accesstoken from TxnCName: ", err)
		}
	}

	errRET = th.tw.removeTokenData(ctx, ti.GetRefresh(), th.tcfg.RefreshCName)
	if errRET != nil {
		log.Println("T2: Failed to remove refreshToken from RefreshCName: ", errRET)
		return
	} else {
		err = th.tw.removeTransactionData(ctx, ti.GetRefresh())
		if err != nil {
			log.Println("T2: Failed remove refreshtoken from TxnCName: ", err)
		}
	}

	errRET = th.tw.removeBasicData(ctx, basic)
	if errRET != nil {
		log.Println("T3: Failed to remove basicToken from BasicCName: ", errRET)
		return
	} else {
		err = th.tw.removeTransactionData(ctx, basic)
		if err != nil {
			log.Println("T3: Failed remove basictoken from TxnCName: ", err)
		}
	}
